...
**coordinator 按轮驱动(一次激活一条活会话),同时是用户的对话界面**

引擎按**轮**驱动 coordinator,但一次激活(activation)内**复用同一条会话**:会话自己记得之前的轮次,
每个后续轮只投递增量(落定任务的台账视图 + 用户新消息 + 预算余量),记忆与推理连续,不付每轮重建的开销。
台账仍是唯一持久状态:会话半路死掉(适配器崩溃、上下文溢出)引擎就地丢弃它,从台账重建一条新会话重试该轮;
进程重启/会话重开同理。重建轮的首个 prompt 带全量上下文——「目标 + **完整用户对话记录**(worker 往来不进对话,
只以台账 summary 出现)+ 自存便签 + 任务台账快照 + 预算余量」——对话历史带尾部窗口截断,单轮上下文随任务树
与对话尾部走,**不随轮数增长**。`record_note`(外置到 run,限 20 条)记的是台账和对话都无法复原的策略与决策。
轮与轮之间由台账事件唤醒(任务落定/新反问/**用户消息**/系统通知)。**轮数不设上限**——终止保证是墙钟;
